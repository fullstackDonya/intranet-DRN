...
// single-tenant: no customer check
?>
<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="utf-8">
  <title>Messagerie ERP</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="../crm/assets/css/chat.css">
...
<div class="container mt-3">
  <h2>Messagerie</h2>
  <div><a class="btn" href="/api/messages.php?action=create_conversation" onclick="event.preventDefault();createConversation();">+ Nouvelle conversation</a></div>
...
(function(){
  ...
  function createConversation(){
    fetch('../api/messages.php?action=create_conversation', { method: 'POST' })
      .then(r=>r.json()).then(d=>{ if (d.id){ loadConversations(); currentConversationId=d.id; loadMessages(d.id); } });
  }
  ...
  if (window.WebitechRealtime){
    const rt = WebitechRealtime({ endpoint: '/api/realtime.php', topics: ['messages','sales'] });
    rt.on('message.created', function(evt){
      const d = evt && evt.data || evt;
    ...
    // sales events could be surfaced as notifications if desired
    rt.start();
  }
